...
class Card:
    def __init__(self, list_input, card_string_len):
        #отсортированные списки из 9 случайных чисел
        self.__random_num_list1 = self.sort_list(list_input[0:9])    
        self.__random_num_list2 = self.sort_list(list_input[9:18])   
        self.__random_num_list3 = self.sort_list(list_input[18:27])
        #длина строки в карте    
        self.__card_string_len = card_string_len          
        #строки с пробелами
        self.__card_string1 = self.get_space_list(self.__random_num_list1)
        self.__card_string2 = self.get_space_list(self.__random_num_list2)
        self.__card_string3 = self.get_space_list(self.__random_num_list3)
    ...
    def is_full(self):
        flag = True
        for i in range(self.__card_string_len):
            if self.__card_string1[i] != 'X':
                flag =  False
                return flag
            elif self.__card_string2[i] != 'X':
                flag =  False
                return flag
            elif self.__card_string3[i] != 'X':
                flag =  False
                return flag
        return flag
    ...
    @staticmethod    
    def get_space_list(num_list):    
        card_string = num_list[:]
        for _ in range(4):
...
def main():
    ...
    user_card = Card(get_random_digits(27), card_string_len) #карта пользователя
    pc_card = Card(get_random_digits(27), card_string_len)  #карта пк 
